/*
 * @Description: file content
 * @version:
 * @Author: ywl
 * @Date: 2020-10-20 15:03:23
 * @LastEditors: wwq
 * @LastEditTime: 2021-02-17 09:33:20
 */
import IhSelectPageBase from './src/select-page-base.vue'
import IhSelectPageOrg from './src/select-page-org.vue'
import IhSelectPageUser from './src/select-page-user.vue'
import IhSelectPageDivision from './src/select-page-division.vue'
import IhSelectPageByChannel from './src/select-page-channel.vue'
import IhSelectPageByProject from './src/select-page-project.vue'
import IhSelectPageByCycle from './src/select-page-cycle.vue'
import IhSelectPageByBuild from './src/select-page-build.vue'
import IhSelectPageByRoom from './src/select-page-room.vue'
import IhSelectPageByCompany from './src/select-page-company.vue'
import IhSelectPageByDeveloper from './src/select-page-developer.vue'
import IhSelectPageByPayer from './src/select-page-payer.vue'
import IhSelectPageByOrgCompany from './src/select-page-payer.vue'

export {
  IhSelectPageByDeveloper,
  IhSelectPageByCompany,
  IhSelectPageDivision,
  IhSelectPageByChannel,
  IhSelectPageByProject,
  IhSelectPageByCycle,
  IhSelectPageByBuild,
  IhSelectPageByRoom,
  IhSelectPageUser,
  IhSelectPageOrg,
  IhSelectPageBase,
  IhSelectPageByPayer,
  IhSelectPageByOrgCompany
}
