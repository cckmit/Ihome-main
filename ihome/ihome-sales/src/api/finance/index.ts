...
/**下载银行网点导入模板*/
export async function get_bankBranch_getExcelTemplate(d?: any) {
  return await request.get<any, any>(basePath + '/bankBranch/getExcelTemplate', { params: d })
}
/**查询银行网点信息列表数据*/
...
export async function get_payment_autoPay__id(d?: any) {
  return await request.get<any, any>(basePath + '/payment/autoPay/{id}', { params: d })
}
/**付款批量解除关联成交报告*/
...
/**根据成交ID获取收款,退款,开票信息*/
export async function get_payment_getPaymentAndRefundAndInvoiceInfo__dealCode(d?: any) {
  return await request.get<PaymentAndRefundAndInvoiceInfoVO, PaymentAndRefundAndInvoiceInfoVO>(basePath + '/payment/getPaymentAndRefundAndInvoiceInfo/{dealCode}', { params: d })
}
/**获取二维码信息*/
...
export async function post_payment_queryOnlinePayTradeResult__id(d?: any) {
  return await request.post<string, string>(basePath + '/payment/queryOnlinePayTradeResult/{id}', d)
}
...
export async function post_payment_syncBusinessId(d?: any) {
  return await request.post<number, number>(basePath + '/payment/syncBusinessId', d)
}
/**手动同步已付收款的电子回单*/
export async function post_payment_syncElectronicReceipt__id(d?: any) {
  return await request.post<boolean, boolean>(basePath + '/payment/syncElectronicReceipt/{id}', d)
}
/**房号确认书签署同步修改收款房号*/
...
/**成交报告终审通过后更新收款订单的确认时间*/
export async function post_payment_updatePayConfirmTime(d?: any) {
  return await request.post<boolean, boolean>(basePath + '/payment/updatePayConfirmTime', d)
}
/**删除草稿状态的POS机事项*/
...
/**下载pos机终端信息导入模板*/
export async function get_posTerminal_getExcelTemplate(d?: any) {
  return await request.get<any, any>(basePath + '/posTerminal/getExcelTemplate', { params: d })
}
/**查询POS机终端信息列表数据*/
...
/**根据成交编号获取所有待退款项退款总金额*/
export async function get_refundItem_getRefundAmountByDealCode__dealCode(d?: any) {
  return await request.get<number, number>(basePath + '/refundItem/getRefundAmountByDealCode/{dealCode}', { params: d })
}
/**根据退款申请单号获取退款申请ID*/
...
/**退款申请书关联成交时候,同步待退款项的成交数据*/
export async function post_refundItem_syncRefundItem(d?: any) {
  return await request.post<boolean, boolean>(basePath + '/refundItem/syncRefundItem', d)
}
/**反馈结果[集中支付低第三步(最后一步)]*/
...
export interface InvoiceHTZPVO {
  /**开票信息ID集合*/
  ids: number[];
  /**发票类型[服务费必传,代理费不传](SpecialElectron-增值税专用发票（电子）、OrdinaryElectron-增值税普通发票（电子）、OrdinaryPaper-增值税普通发票（纸质)、SpecialPaper-增值税专用发票（纸质）)*/
  operationType: string;
}
/**InvoiceHandVO*/
...
/**PaymentAndRefundAndInvoiceInfoVO*/
export interface PaymentAndRefundAndInvoiceInfoVO {
  /**开票信息*/
...
export interface PaymentRefundVO {
  ...
  /**退款时间(yyyy-MM-dd)*/
  refundedDate: string;
  /**状态(PendingApply-待申请、UnderReview-审核中、Refunding-退款中、Refunded-已退款)*/
  status: string;
}
...
export interface PosApplyDetailVO {
  ...
  /**事项编号*/
  itemNo: string;
  /**事项类别(Use-财务领用、Apply-业务申领、Move-业务调动、Return-财务退还、GiveBack-业务归还)*/
  itemType: string;
  /**undefined*/
  ...
  /**状态(CentralStock-总部库存、UseWaitApprove-财务领用待审、UseWaitSend-财务领用待寄、UseOneTheWay-财务领用在途、ReturnOnTheWay-财务退还在途、BranchStock-分公司库存、ApplyWaitApprove-业务申领待审、ApplyWaitSend-业务申领待寄、ApplyOnTheWay-业务申领在途、Using-项目在用、GiveBackOnTheWay-业务归还在途、MoveWaitApprove-业务调动待审、MoveWaitSend-业务调动待寄、MoveOnTheWay-业务调动在途、Stop-停用、Draft-草稿、Finished-已结束、Terminated-已终止)*/
  status: string;
}
/**PosApplyItemQueryVO*/
...
export interface PosApplyItemRecordVO {
  ...
  /**操作时间(yyyy-MM-dd HH:mm:ss)*/
  operateTime: string;
  /**操作(PutIn-入库、Revocation-撤机、Modify-修改、Use-财务领用、UseApprove-财务领用审核、UseSend-财务领用寄出、UseSign-财务领用签收、Return-财务退还、ReturnConfirm-确认财务退还、Apply-业务申领、ApplyApprove-业务申领审核、ApplySend-业务申领寄出、ApplySign-业务申领签收、GiveBack-业务归还、GiveBackConfirm-确认业务归还、Move-业务调动、MoveApprove-业务调动审核、MoveSend-业务调动寄出、MoveSign-业务调动签收)*/
  operation: string;
  /**操作人*/
  operator: number;
  /**操作人*/
...
export interface PosApplyOperateVO {
  /**事项ID*/
  id: number;
  /**事项操作类型(PutIn-入库、Revocation-撤机、Modify-修改、Use-财务领用、UseApprove-财务领用审核、UseSend-财务领用寄出、UseSign-财务领用签收、Return-财务退还、ReturnConfirm-确认财务退还、Apply-业务申领、ApplyApprove-业务申领审核、ApplySend-业务申领寄出、ApplySign-业务申领签收、GiveBack-业务归还、GiveBackConfirm-确认业务归还、Move-业务调动、MoveApprove-业务调动审核、MoveSend-业务调动寄出、MoveSign-业务调动签收)*/
  operateType: string;
...
export interface RefundItemPushQueryVO {
  ...
  /**付款方ID*/
  companyId: number;
  /**日期类型[-申请日期;推送日期;付款日期](ApplicationDate-申请日期、PushDate-推送日期、PaymentDate-付款日期)*/
  dateType: string;
  /**截止时间(yyyy-MM-dd)*/
  endTime: string;
  /**(必填)当前页*/
...
/**RefundItemSyncDealVO*/
export interface RefundItemSyncDealVO {
  /**服务费实收金额*/
  actualAmount: number;
  /**优惠告知书ID*/
  businessId: number;
...
/**UpdatePayConfirmTimeVO*/
export interface UpdatePayConfirmTimeVO {
  /**成交业绩确认时间(yyyy-MM-dd HH:mm:ss)*/
  confirmTime: string;
  /**成交ID*/
  dealId: number;
}
