 ...
 * @LastEditors: wwq
 * @LastEditTime: 2020-12-10 16:51:30
-->
<template>
  <el-dialog
    v-dialogDrag
  ...
  >
    ...
    >
      ...
      <tr>
        <td rowspan="2">客户成交及确认</td>
        <td>甲方</td>
        <td colspan="4">{{info.jfCustomerConfirm}}</td>
      </tr>
      <tr>
        <td>渠道</td>
        <td colspan="4">{{info.qdCustomerConfirm}}</td>
      </tr>
      <tr>
        <td rowspan="2">违约责任</td>
        <td>甲方</td>
        <td colspan="4">{{info.jfResponsibiltity}}</td>
      </tr>
      <tr>
        <td>渠道</td>
...
import { get_term_getOAContractFrom__termId } from "@/api/project/index";
...
export default class ViewContract extends Vue {
  ...
  async getInfo() {
    const item = await get_term_getOAContractFrom__termId({
