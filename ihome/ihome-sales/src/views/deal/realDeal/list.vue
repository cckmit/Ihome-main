      ...
      switch (btnName) {
        case "UPDATE":
          ...
          if (row.id === row.parentId) {
            ...
            if (
              row.status === "Draft" &&
              (this as any).$roleTool.RProjectSite()
            ) {
              // 草稿、案场
              flag = false;
            }
            if (
              ["AchieveDeclareUnconfirm", "Reject"].includes(row.status) &&
              (this as any).$roleTool.RFrontLineClerk()
            ) {
              // 业绩申报待确认 + 驳回、文员
              flag = false;
            }
            ...
            // 补充成交
            if (
              ["Draft", "Reject"].includes(row.status) &&
              (this as any).$roleTool.RFrontLineClerk()
            ) {
              // 草稿 + 驳回、文员
              flag = false;
        ...
        case "REVOKE":
          // 撤回按钮权限
          if (
            row.status === "PlatformClerkUnreview" &&
            (this as any).$roleTool.RFrontLineClerk()
          ) {
            // 平台文员待审核、文员
            flag = false;
          ...
          if (
            row.status === "HeadDepartUnreview" &&
            (this as any).$roleTool.RPlatformClerk()
          ) {
            // 事业部负责人待审核、平台文员
            flag = false;
          }
          if (
            row.status === "BranchBusinessManageUnreview" &&
            (this as any).$roleTool.RDepartmentLeader()
          ) {
            // 分公司业管待审核、事业部负责人
            flag = false;
          }
          ...
          if (
            ...
            (this as any).$roleTool.RBusinessManagement()
          ) {
        ...
        case "VERIFY":
          ...
          if (
            row.status === "HeadDepartUnreview" &&
            this.$roleTool.RDepartmentLeader()
          ) {
            // 事业部负责人待审核、事业部负责人
            flag = false;
          }
          if (
            row.status === "BranchBusinessManageUnreview" &&
            this.$roleTool.RBusinessManagement()
          ) {
