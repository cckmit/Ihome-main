 ...
 * @LastEditors: lsj
 * @LastEditTime: 2021-04-08 08:41:22
-->
<template>
...
<script lang="ts">
  ...
  import {
    ...
    post_pageData_getMingYuanData, // 编辑功能 - 根据项目周期及房号，查询明源数据
  ...
  export default class EntryDealInfo extends Vue {
    ...
    canAddNoticeFlag: any = true; // 是否有添加(删除)优惠告知书的标识：true-可以；false-不可以
    // 编辑功能相关字段
    ...
    async editInitPage(id: any) {
      ...
      (this as any).$nextTick(async () => {
        ...
        this.postData.firstContNo = res.firstContNo;
        ...
        await this.editInitPageById(res);
      });
    }
    ...
    async editInitPageById(res: any = {}) {
      // if (!cycleId || !roomId || !propertyType || !refineModel) return;
      let params: any = {
        parentId: res?.parentId, // 编辑要传主成交id
        cycleId: res?.cycleId,
    ...
    getPostData() {
      ...
      if (this.postData.receiveVO && this.postData.receiveVO.length) {
        ...
        if (serviceItem && serviceItem.length) {
          obj.dealVO.receiveServiceFee = serviceItem[0].receiveAmount;
        } else {
      ...
      obj.dealVO.firstContNo = this.postData.firstContNo;
      // 优惠告知书
      ...
      // obj.dealVO.reportId = this.baseInfoInDeal.recordId; // 2021-05-08
      obj.dealVO.reportId = this.postData.roomId ? this.postData.reportId : null;
