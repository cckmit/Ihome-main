  ...
  <ih-page class="text-left">
    ...
    >
      <div class="add-all-wrapper">
        <el-button
          type="success"
          @click="addPerformance('TotalBag')"
        >添加总包</el-button>
        <el-button
          type="success"
          @click="addPerformance('Distri')"
        >添加分销</el-button>
      </div>
      <el-table
        class="ih-table"
...
export default class AchieveScaleSchemeAdd extends Vue {
  ...
  rules: any = {
    modelId: [{ required: true, message: "业务模式必选", trigger: "change" }],
  ...
  private checkRatio: any = (rule: any, value: any, callback: any) => {
    ...
    if (!value) {
      ...
      if (this.tableModel.tableData.length > 0) {
        ...
        if (
          currentType === "TotalBag" &&
          !this.$tool.eq(TotalBagRatioTotal, 100)
        ) {
          return callback(new Error("总包方拆分比例合计必须为100%"));
        } else if (
          currentType === "Distri" &&
          !this.$tool.eq(DistriRatioTotal, 100)
        ) {
  ...
  async getModelContTypeList() {
    ...
    if (!modelName) return;
    const res: any = await get_businessModel_getByName__modelName({
      modelName: modelName,
    });
  ...
  addTotalPage() {
    if (this.tableModel.tableData && this.tableModel.tableData.length > 0) {
      ...
      if (lastIndex !== null) {
        this.tableModel.tableData.splice(
          lastIndex + 1,
          0,
          this.getObj("TotalBag")
        );
      } else {
    ...
    try {
      ...
      if (this.tableModel.tableData && this.tableModel.tableData.length > 0) {
        if (this.postData.isSame === "Yes") {
          ...
          // 删除总包/分销
          this.tableModel.tableData = this.tableModel.tableData.filter(
            (data: any) => {
